...
def dataset_stats():
    ...
    fig, ax = plt.subplots()
    long = len([0 for s in scores_raw if s['reason'] == 'Longitudinally too close'])
    lat = len([0 for s in scores_raw if s['reason'] == 'Laterally too close'])
    other = len([0 for s in scores_raw if s['reason'] == 'Too close'])
    reasons = ['Longitudinally too close', 'Laterally too close', 'Too close']
    values = [long, lat, other]
    ...
    ax.set_ylabel('Lateral distance')
